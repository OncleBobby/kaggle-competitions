...
split_train_set_node = node(func=split_train_set, \
    inputs=['x_model', 'y_model', 'params:parameters'], \
    outputs=['x_train', 'y_train', 'x_test', 'y_test'], tags=['preparation'])
calibrate_model_node = node(func=calibrate, \
    inputs=['params:parameters', 'x_train', 'y_train', 'x_test', 'y_test'], \
    outputs=['model_fraud', 'model_fraud_name', 'model_score_fraud'], tags=['calibration'])
